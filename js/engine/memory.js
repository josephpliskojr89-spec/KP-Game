...
(function (root) {
  ...
  KP.narrativeText = function (state, n) {
    ...
    switch (n.key) {
      ...
      case 'ownMoney': return idol() + '’s songwriting pays directly now — publishing money that follows the person, not the contract. Renewal tables know it.';
      case 'playedHurt': return idol() + ' performs through what the reports said to rest. The fandom keeps the receipts; the company kept the schedule.';
      case 'domeNight': return group() + ' played the dome. Two kinds of touring acts exist; they are the other kind now.';
  ...
  function formationLine(state, n) {
    ...
    switch (n.key) {
      ...
      case 'ownMoney': return KP.fillPro('The quietest kind of career news: ' + idol() + '’s publishing royalties crossed the line from rounding error to income. The songs {she} wrote pay {her} every quarter, on paper that has nothing to do with any label. People who own their own money sit differently at renewal tables, and everyone at those tables knows it.', state.people[n.subjectId]);
      case 'playedHurt': return KP.fillPro('The compilation exists now: ' + idol() + ' working stages the medical reports said to sit out — taped, seated, smiling. The fandom titles it devotion and means it as evidence. Every heavy schedule the company books {her} from here gets read against that folder, and the folder only grows.', state.people[n.subjectId]);
      case 'domeNight': return 'The industry keeps one list that never needs explaining: acts that have played a Japanese dome. ' + group() + ' is on it now — fifty thousand tickets, counted, not argued. Every venue conversation this company ever has again starts from a different floor.';
