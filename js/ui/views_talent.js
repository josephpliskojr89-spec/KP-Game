...
(function (root) {
  ...
  UI.renderDossier = function (state, id, tab) {
    ...
    if (p.status === 'idol') {
      ...
      // the career, in-house (v0.10.25, §87 F): the launch keeps her —
      // no label volunteers its star out of the lineup
      if (inRealGroup) {
        if (p.dualCareer) {
          row.push('<span class="chip" style="align-self:center">Solo career open — in-house since ' + UI.esc(KP.weekLabel(p.dualCareer.since).text) + '</span>');
        } else {
          row.push('<button class="btn small ghost" style="border:1px solid var(--line)" data-action="launch-solo" data-id="' + p.id + '">Open the solo career — in-house</button>');
        }
      }
